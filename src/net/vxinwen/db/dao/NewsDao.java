...
public class NewsDao {
	 ...
	 */
	private final static String[] COLUMNS_INSERTED = new String[] { "title",
			"summary", "image_address", "url", "publish_time", "category" };
	...
	public List<News> getByCategory(Context context, String category) {
		...
		try {
			...
			cursor = db.query(TABLE, null, "category=?",
			...
			if (cursor.moveToFirst()) {
				do {
					...
					news.setImageAddress(cursor.getString(cursor
							.getColumnIndex("image_address")));
					news.setUrl(cursor.getString(cursor.getColumnIndex("url")));
					Timestamp publishTime = TimestampUtil
							.stringToTimeStamp(cursor.getString(cursor
	...
	public boolean insertBatch(Context context, List<News> newses) {
		...
		try {
			db.beginTransaction();
			db = new DBOpenHelper(context).getWritableDatabase();
			String sql = getInsertBatchSql(TABLE, COLUMNS_INSERTED,
					toNewsArray(newses));
			db.execSQL(sql);
			db.endTransaction();
			return true;
	...
	private String[][] toNewsArray(List<News> newses) {
		...
		for (int i = 0; i < size; i++) {
			...
			newsArray[i][4] = TimestampUtil.timeStampToString(news
					.getPublishTime());
	 ...
	 * @return 拼接好的sql语句。
	 */
	private String getInsertBatchSql(String tableName, String[] columns,
			String[][] values) {
