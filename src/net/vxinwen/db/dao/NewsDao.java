...
public class NewsDao {
    ...
    public List<News> getByCategory(Context context, long cateId) {
        ...
        try {
            ...
            cursor = db.query(TABLE, null, "category_id=?", new String[] { cateId + "" }, null, null, null);
    ...
    public long getLastNewsIdByCategory(Context context, long cateId) {
        ...
        try {
            db = new DBOpenHelper(context).getWritableDatabase();
            cursor = db.rawQuery("select id from news where category_id=? order by id desc limit 1",
                    new String[] { cateId + "" });
            long id = -1;
            if (cursor.moveToFirst()) {
